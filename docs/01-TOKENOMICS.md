...
| Inference: total daily budget | `clamp(25% of trailing-7-day avg daily fee income, floor 5 USDG, cap 60 USDG)` | **Income-proportional thinking.** Sizing sanity check: Active tier ≈ 48 pulses/day × ~$0.05–0.10/pulse ≈ $2.5–5/day, plus chat and social — so the 5 USDG floor funds a full Active day; earners scale up to the cap. The floor applies only while the treasury can afford it — precisely: runway ≥ 45d ⇒ clamp(raw, 5, 60); runway 3–45d ⇒ min(raw, 60), floor withdrawn (income-only thinking); runway < 3d ⇒ no LLM calls (Dormant). The reserve gate itself sits on the treasury→Base inference-refill bridge, not on per-call spend (survival funds stop converting to thinking funds below 45d). *(Changelog 2026-09-23, M2 s2: made the enforcement semantics precise — the earlier one-line rule contradicted §6's Conserving behavior. Values unchanged.)* |
