...
4. **OpenRouter**: provision a key via API, top up via crypto payments API with a small amount, make a call. (Needs Juan: OpenRouter org + ~$20.)
...
`runtime/` skeleton: keyring (mock KMS locally), policy engine (+ its full test suite — the most important code in the project), pulse machine vs mock LLM + local chain fork, memory + snapshot/restore, treasury daemon. No Phala yet.
...
Dockerize reproducibly; deploy to Phala testnet CVM; real KMS keyring; attestation → Arweave; `genesis/` orchestrator end-to-end on testnet: website-less genesis via script → agent live, casting on Farcaster (testnet-flagged account), journaling on Arweave, trading on testnet pools. Kill/restore drill with real KMS. (Needs Juan: Phala account/billing.)
