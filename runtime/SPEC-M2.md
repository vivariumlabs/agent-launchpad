...
- **T0 (rev 2, M2 s2 review — reconciles 01 §5 with 01 §6):** the 45-day reserve gates ONLY the outflows that drain `fundable`: `allowance`, and `acrossBridge` with asset USDG or USDC. Those must leave `runwayDays ≥ cfg.minRunwayDays (45 DEFAULT)` after the spend, else `deny(RUNWAY)`. EXEMPT: `oysterRental` (raises runway), `gasTopUp` and ETH-asset bridges (survival infrastructure — heartbeats and rental txs need gas; tightly capped per day), `arweaveFunding` (snapshots must persist precisely when the agent is dying, 03 §9; 10 USDG/day cap), `x402Data` and `inference` (spend already-bridged Base USDC; the REFILL bridge is where the reserve gate bites — survival funds stop converting to thinking funds below 45d). Rationale: rev 1's blanket gate made Conserving agents (3–14d) unable to think, contradicting 01 §6, and would have starved heartbeats of gas.
...
Inference:
- **I1 (rev 2)** Daily budget: `raw = 25% × avg(feeIncome7d)`; if `runwayDays ≥ 45`: `B = clamp(raw, 5 USDG, 60 USDG)`; if `3 ≤ runwayDays < 45`: `B = min(raw, 60 USDG)` — the floor is withdrawn ("the floor applies only while the treasury can afford it", 01 §5): low-reserve agents think only from actual income; if `runwayDays < cfg.dormantRunwayDays (3 DEFAULT)`: `deny(RUNWAY)` — Dormant = no LLM calls (01 §6). Category budget = `B × weight[category]`. Allow iff `inferenceSpent[cat] + maxCostUsd ≤ catBudget` (`deny(INFERENCE_BUDGET)`). No inter-category borrowing.
...
- **A2** Per-counterparty daily cap — `actionTransfer` only: for USDG, `counterpartySpent[to][USDG] + amount ≤ counterpartyPctBps (3000 DEFAULT) × allowanceAmountToday`; for any other asset, `≤ 3000 bps × action balance of that asset at first send of the day` (reducer snapshots the denominator in the ledger on first spend: `counterpartySpent[to][asset+":denom"]`). Else `deny(COUNTERPARTY_CAP)`. Swaps/LP route to canonical contracts (`cfg.poolManager`) and are exempt from A2 (still A1-capped). **rev 2:** `actionMint` is A2-CAPPED (target-keyed, "ETH" assetKey, same denominator rules) — mint `value` to an arbitrary target was a K×20% per-pulse drain vector otherwise (caught in M2 s2 review).
